...
namespace network
{
template ReactionNetwork<UO2CsReactionNetwork>::ReactionNetwork();
...
template double
ReactionNetwork<UO2CsReactionNetwork>::getTotalConcentration(
	ConcentrationsView concentrations, Species type, AmountType minSize);

template double
ReactionNetwork<UO2CsReactionNetwork>::getTotalRadiusConcentration(
	ConcentrationsView concentrations, Species type, AmountType minSize);

template double
ReactionNetwork<UO2CsReactionNetwork>::getTotalAtomConcentration(
	ConcentrationsView concentrations, Species type, AmountType minSize);

template double
ReactionNetwork<UO2CsReactionNetwork>::getTotalTrappedAtomConcentration(
	ConcentrationsView concentrations, Species type, AmountType minSize);

template double
ReactionNetwork<UO2CsReactionNetwork>::getTotalVolumeFraction(
	ConcentrationsView concentrations, Species type, AmountType minSize);

double
UO2CsReactionNetwork::checkLatticeParameter(double latticeParameter)
{
...
double
UO2CsReactionNetwork::checkImpurityRadius(double impurityRadius)
{
...
UO2CsReactionNetwork::IndexType
UO2CsReactionNetwork::checkLargestClusterId()
{
	// Copy the cluster data for the parallel loop
	auto clData = _clusterData.d_view;
	using Reducer = Kokkos::MaxLoc<UO2CsReactionNetwork::AmountType,
		UO2CsReactionNetwork::IndexType>;
	Reducer::value_type maxLoc;
	Kokkos::parallel_reduce(
		"UO2CsReactionNetwork::checkLargestClusterId", _numClusters,
		KOKKOS_LAMBDA(IndexType i, Reducer::value_type & update) {
			const Region& clReg = clData().getCluster(i).getRegion();
			Composition hi = clReg.getUpperLimitPoint();
			auto size = hi[Species::Cs] + hi[Species::V];
			if (size > update.val) {
				update.val = size;
				update.loc = i;
			}
...
void
UO2CsReactionNetwork::initializeExtraClusterData(const options::IOptions& options)
{
...
void
UO2CsReactionNetwork::readClusters(const std::string filename)
{
...
void
UO2CsReactionNetwork::readReactions(double temperature, const std::string filename)
{
	...
	while (tokens.size() > 0) {
		...
		comp[Species::Cs] = static_cast<IndexType>(tokens[0]);
	...
	while (tokens.size() > 0) {
		...
		// Production case
			...
			comp[Species::Cs] = static_cast<IndexType>(tokens[3]);
			...
			comp[Species::Cs] = static_cast<IndexType>(tokens[6]);
	...
	if (rId != this->invalidIndex()) {
		auto mapId = fileClusterMap.value_at(fileClusterMap.find(rId));
		// Save its formation energy
		clData().setCsFormationEnergy(g0Vector[mapId]);
	}
