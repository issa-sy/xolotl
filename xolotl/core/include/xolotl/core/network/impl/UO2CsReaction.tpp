...
KOKKOS_INLINE_FUNCTION
double
UO2CsProductionReaction::getRateForProduction(IndexType gridIndex)
{
...
KOKKOS_INLINE_FUNCTION
double
UO2CsProductionReaction::computeRate(IndexType gridIndex, double time)
{
	auto cl0 = this->_clusterData->getCluster(_reactants[0]);
	auto cl1 = this->_clusterData->getCluster(_reactants[1]);
...
KOKKOS_INLINE_FUNCTION
void
UO2CsProductionReaction::computeFlux(
...
KOKKOS_INLINE_FUNCTION
void
UO2CsProductionReaction::computePartialDerivatives(
...
KOKKOS_INLINE_FUNCTION
void
UO2CsProductionReaction::computeReducedPartialDerivatives(
	ConcentrationsView concentrations, Kokkos::View<double*> values,
	IndexType gridIndex)
{
	constexpr auto speciesRangeNoI = NetworkType::getSpeciesRangeNoI();
...
KOKKOS_INLINE_FUNCTION
double
UO2CsDissociationReaction::computeBindingEnergy()
{
	...
	auto csFormation = this->_clusterData->getCsFormationEnergy();
	...
	if (prod1Comp.isOnAxis(Species::V) or prod2Comp.isOnAxis(Species::V)) {
		double csSDPower = csSD * csSD;
		be = 0.0;
		for (auto i = 0; i < 8; i++) {
			be += (double)(i + 1) * fitParams[7 - i] * csSDPower;
			csSDPower *= csSD;
		}
	...
	if (prod1Comp.isOnAxis(Species::Cs) or prod2Comp.isOnAxis(Species::Cs)) {
		double csSDPower = csSD;
		be = 0.0;
		for (auto i = 0; i < 8; i++) {
			be -= (double)(i + 2) * fitParams[7 - i] * csSDPower;
			csSDPower *= csSD;
		}
		be += csFormation;
	}
...
KOKKOS_INLINE_FUNCTION
double
UO2CsSinkReaction::getSinkStrength()
{
	// Not actually used
	//	return 1.0e-3;
...
KOKKOS_INLINE_FUNCTION
void
UO2CsSinkReaction::computePartialDerivatives(ConcentrationsView concentrations,
...
KOKKOS_INLINE_FUNCTION
void
UO2CsSinkReaction::computeReducedPartialDerivatives(
	ConcentrationsView concentrations, Kokkos::View<double*> values,
	IndexType gridIndex)
{
	auto& clusterMap = this->_clusterData->extraData.fileClusterMap;
