...
#include <xolotl/core/network/impl/UO2CsClusterGenerator.tpp>
#include <xolotl/core/network/impl/UO2CsReaction.tpp>
...
void
UO2CsReactionGenerator::operator()(IndexType i, IndexType j, TTag tag) const
{
	...
	if ((lo1[Species::Cs] == 1 and lo1[Species::V] == 1 and
			lo2[Species::Cs] > 0 and lo2[Species::V] > 0) or
		(lo2[Species::Cs] == 1 and lo2[Species::V] == 1 and
			lo1[Species::Cs] > 0 and lo1[Species::V] > 0)) {
	...
	for (IndexType k = 0; k < numClusters; ++k) {
		...
		if (isGood) {
			...
			// Dissociation
			if (lo1[Species::Cs] + lo1[Species::V] + lo1[Species::I] < 3 ||
				lo2[Species::Cs] + lo2[Species::V] + lo2[Species::I] < 3) {
...
KOKKOS_INLINE_FUNCTION
void
UO2CsReactionGenerator::addSinks(IndexType i, TTag tag) const
{
	using Species = typename NetworkType::Species;
	using Composition = typename NetworkType::Composition;
...
inline ReactionCollection<UO2CsReactionGenerator::NetworkType>
UO2CsReactionGenerator::getReactionCollection() const
{
...
inline detail::UO2CsReactionGenerator
UO2CsReactionNetwork::getReactionGenerator() const noexcept
{
	return detail::UO2CsReactionGenerator{*this};
}
...
KOKKOS_INLINE_FUNCTION
void
UO2CsClusterUpdater::updateDiffusionCoefficient(
	const ClusterData& data, IndexType clusterId, IndexType gridIndex) const
{
	if (data.migrationEnergy(clusterId) > 0.0) {
		...
		//		Composition lo = clReg.getOrigin();
		//		if (clReg.isSimplex() && lo[Species::V] == 1 && lo[Species::Cs]
		//== 1) {
		//			// Intrinsic diffusion
