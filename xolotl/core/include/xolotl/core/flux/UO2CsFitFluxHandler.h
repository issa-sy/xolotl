 ...
 * cesium flux for nuclear fuel.
 */
class UO2CsFitFluxHandler : public FluxHandler
{
private:
	/**
	 * \see FluxHandler.h
...
public:
	 ...
	 * The constructor
	 */
	UO2CsFitFluxHandler(const options::IOptions& options) :
		FluxHandler(options),
		csYield(options.getFissionYield()),
		defectYield(1.0e4),
		...
		std::vector<double> grid) override
	{
		...
		// Set the flux index corresponding the the single cesium cluster here
		using NetworkType = network::UO2CsReactionNetwork;
		...
		comp[NetworkType::Species::Cs] = 1;
		cluster = neNetwork.findCluster(comp, plsm::HostMemSpace{});
		// Check that the cesium cluster is present in the network
		if (cluster.getId() == NetworkType::invalidIndex()) {
			fluxIndices.push_back(NetworkType::invalidIndex());
		}
		else {
		...
		Kokkos::parallel_for(
			1, KOKKOS_LAMBDA(std::size_t) {
				...
				if (ids[2] != NetworkType::invalidIndex()) {
					updatedConcOffset[ids[2]] += cesiumYield * amplitude; // Cs
				}
			});
	}
